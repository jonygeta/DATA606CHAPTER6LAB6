---
title: 'DATA 606 CHAPTER 6 LAB (Inference for categorical data)'
...
Answer: Those percentages appear to be *sample statistics*.
...
2.  The title of the report is "Global Index of Religiosity and Atheism". To
    ...
    assumption?
      ...
      Answer: We must assume that the observations are random and sample drawn using simple random sampling method.
## The data
...
Answer:
      Each row of table 6 represents a survey observation countries percentages religion data.
    Each row of atheism represents atheism survey single observation.
...
Proportions of atheist is 0.05 % which agrees with the table 6.
...
Answer:
Conditions for inference 1. Sample observations are independent and 2. We can calculate the standard error and construct the interval using general formula or use the inference function.

The sample is drawn using simple random sample hence they're independent. 
For second condition we can calculate the standard error and construct interval by inference function below.
...
```
 ...
 0.013524 is the margin of error.
...
```{r}
inference(atheism[atheism$nationality=="Armenia" & atheism$year == "2012","response"], est = "proportion", type = "ci", method = "theoretical", 
          success = "atheist")

```
Margin of error for Armenia
```{r}
0.0063 *1.96
```
...
```{r}
inference(atheism[atheism$nationality=="Canada" & atheism$year == "2012","response"], est = "proportion", type = "ci", method = "theoretical", 
          success = "atheist")
...
Condition for inference are met as the samples are independent and drawn using simple random sapling.
...
Answer:
me increase when p increase and reaches a maximum value at p = 0.5 and me decreases for p larger than 0.5.
...
The sampling distribution is approximately normally distributed with the mean close to the population mean 0.1. There are few outliers on the end of the sample data.
...
hist(simulate(400,0.1), main = "p = 0.1, n = 400")
hist(simulate(1040,0.02), main = "p = 0.02, n = 1040")
hist(simulate(400,0.02), main = "p = 0.02, n = 400")
par(mfrow = c(1,1))
```
The larger the value of n the smaller the spread and margin of error. For p larger the value of p smaller the spread .
...
We should not process with inference for Ecuador. We can proceed for Australia. 
* * *
## On your own
...
Answer:
Null Hypothesis: Atheism index of span is equal for 2005 and 2012
Alternative Hypothesis: Atheism index of Spain is not same for 2005 and 2012
...
```{r}
inference(spain[spain$year == "2005","response"], est = "proportion", type = "ci", method = "theoretical", 
          success = "atheist")
```

Confidence interval for 2012
```{r}
inference(spain[spain$year == "2012","response"], est = "proportion", type = "ci", method = "theoretical", 
          success = "atheist")
```
   In 2005 , Spain atheist confidence interval is (0.083,0.1177) and in 2012 (0.0734,0.1065). Since the confidence interval overlap I conclude that there is no significant difference of atheism index of Spain between 2005 and 2012, but it could be due to chance. We accept the null hypothesis.
...
Null Hypothesis: Atheism index of United States is equal for 2005 and 2012
Alternative Hypothesis: Atheism index of United States is not same for 2005 and 2012
...
Confidence interval for 2005
```{r}
inference(us[us$year == "2005","response"], est = "proportion", type = "ci", method = "theoretical", 
          success = "atheist")
```

Confidence interval for 2012
```{r}
inference(us[us$year == "2012","response"], est = "proportion", type = "ci", method = "theoretical", 
          success = "atheist")
```
   In 2005 , United States atheist confidence interval is (0.0038,0.0161) and in 2012 (0.0364,0.0634). Since the confidence interval don't overlap I conclude that there is significant difference of atheism index of United States between 2005 and 2012, but it could be due to chance. We reject the null hypothesis.
...
Answer:
I would expect 5% of those countries failing to detect a change over time due to type 1 error at 5% level of significance.
...
Answer: 
If we've no idea we'll choose p = 0.05
```{r}
p <- 0.5
me <- 0.01
n <- (p-p^2)/(me/1.96)^2
round(n)
```
We've to sample 9604 people to stay within the guidelines.